...
public class ConnectionPool {
	...
	public ConnectionPool(String dbname){
	  ...
	  try {
	  	if(document == null)
	    	document = saxReader.read(ConnectionPool.class.getClassLoader().getResourceAsStream(filename));
	  	Element resreg = document.getRootElement();
	  	List<?> dbResInfos = resreg.elements("dbResInfo");
	    Element dbResInfo = resreg.element("dbResInfo");
	    for(int i=0; i<dbResInfos.size(); i++){
	    	dbResInfo = (Element) dbResInfos.get(i);
	    	if(dbResInfo.element("dbResQname").element("localpart").getText().equals(dbname))
	    		break;
	    }
	    Element dbRes = dbResInfo.element("localDbResType");
	    this.url = dbRes.elementText("jdbcUrl");
	    this.user = dbRes.elementText("jdbcUser");
	    this.pwd = dbRes.elementText("jdbcPwd");
	    this.driver = dbRes.elementText("jdbcDriver");
	    this.maxConn = 100;
	    this.minConn = 50;
	    this.idleTime = 50;
	    this.name = dbResInfo.element("dbResQname").elementText("label");
...
//get connection for the DB
	...
	public void finalize(){
		Iterator<Connection> iterator = connPool.iterator();
